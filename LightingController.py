...
class LightingController:
    ...
    def start(self, numLightHours):
        while (1):  # loop system forever
            if datetime.now().time() > LED_MIN_START_TIME:
                GPIO.output(self.ledPin, GPIO.HIGH)
                print("Lights On!")

                # determine ideal end time, convert to seconds
                endTime = LED_MIN_START_TIME + timedelta(hours=numLightHours)
                # check when we have reached our number of hours or end time
                while ((datetime.now().time() < LED_MAX_END_TIME)
                       and datetime.now().time() < endTime):
                    continue
