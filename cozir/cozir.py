...
communication is based on USB-TTL cable which connects the sensor to the
computer
...
class Cozir(object):
    '''a COZIR CO2 sensor, connected through a USB-TTL cable to a virtual
    serial port
    ...
    def set_mode(self, mode):
        '''set operating mode: command, streaming or polling
        (using OpModes enum)'''
    ...
    def read_firmware(self):
        ...
        # split firmware and serial number
    ...
    def set_filter(self, val):
        '''set the setting of the digital filter to `val`,
        between 0 and 65535'''
        val = int(val)
        assert 0 <= val <= (2**16 - 1)
